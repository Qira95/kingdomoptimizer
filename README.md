...
1. **Data pipeline** (`scripts/`, plain Node, zero dependencies, run manually):
   ...
   - `computeBuildOrder.mjs` runs the greedy cheapest-res-per-CP algorithm over that data (force-building requirement chains when the combined cost is still best, implicit warehouse/granary levels sized to each cost) and writes `data/buildOrder.json`. It replays the finished order to assert every step's requirements are met.
2. **Vue 3 SPA** (`src/`, Vite + Pinia): the optimizer view filters `buildOrder.json` by village context (tribe / capital / city / mutually-exclusive buildings — all derived from the extracted prerequisite data), and the calculator view applies the official time/cost/effect formulas ported from the bundle.
